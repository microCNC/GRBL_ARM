...
void settings_store_startup_line(uint8_t n, char *line)
{
  ...
  write_to_storage(addr,(char*)line, LINE_BUFFER_SIZE);
}
...
{
  write_to_storage(EEPROM_ADDR_BUILD_INFO,(char*)line, LINE_BUFFER_SIZE);
}
...
{  
  ...
  write_to_storage(addr,(char*)coord_data, sizeof(float)*N_AXIS);
}  
...
{
#ifdef FLASH_STORAGE
  Flash_Clear(EEPROM_ADDR_VERSION);
  Flash_Clear(EEPROM_ADDR_GLOBAL);
  Flash_Write(EEPROM_ADDR_VERSION, SETTINGS_VERSION);
#else
  EEPROM_Write(EEPROM_ADDR_VERSION, SETTINGS_VERSION);
#endif
  write_to_storage(EEPROM_ADDR_GLOBAL, (char*)&settings, sizeof(settings_t));
}
...
void settings_clear_startup_lines() {
  #if N_STARTUP_LINE > 0
    #ifdef FLASH_STORAGE
      Flash_Clear(EEPROM_ADDR_STARTUP_BLOCK);
      Flash_Clear(EEPROM_ADDR_STARTUP_BLOCK);
      Flash_Write(EEPROM_ADDR_STARTUP_BLOCK, 0);
    #else
      EEPROM_Write(EEPROM_ADDR_STARTUP_BLOCK, 0);
    #endif
  #endif
  #if N_STARTUP_LINE > 1
    #ifdef FLASH_STORAGE
      Flash_Clear(EEPROM_ADDR_STARTUP_BLOCK+(LINE_BUFFER_SIZE+1));
      Flash_Clear(EEPROM_ADDR_STARTUP_BLOCK+(LINE_BUFFER_SIZE+1));
      Flash_Write(EEPROM_ADDR_STARTUP_BLOCK+(LINE_BUFFER_SIZE+1), 0);
    #else
      EEPROM_Write(EEPROM_ADDR_STARTUP_BLOCK+(LINE_BUFFER_SIZE+1), 0);
    #endif
  #endif
}
...
void settings_clear_build_info() { 
    #ifdef FLASH_STORAGE
      Flash_Clear(EEPROM_ADDR_BUILD_INFO);
      Flash_Clear(EEPROM_ADDR_BUILD_INFO);
      Flash_Write(EEPROM_ADDR_BUILD_INFO, 0);
    #else
      EEPROM_Write(EEPROM_ADDR_BUILD_INFO, 0);
    #endif
  }
...
{
  uint32_t addr = n*(LINE_BUFFER_SIZE+1)+EEPROM_ADDR_STARTUP_BLOCK;
  if (!(read_from_storage((char*)line, addr, LINE_BUFFER_SIZE))) {
...
{
  if (!(read_from_storage((char*)line, EEPROM_ADDR_BUILD_INFO, LINE_BUFFER_SIZE))) {
    // Reset line with default value
    line[0] = 0; // Empty line
...
uint8_t settings_read_coord_data(uint8_t coord_select, float *coord_data)
{
  ...
  if (!(read_from_storage((char*)coord_data, addr, sizeof(float)*N_AXIS))) {
...
uint8_t read_global_settings() {
  ...
  //version = eeprom_get_word(EEPROM_ADDR_VERSION);
  #ifdef FLASH_STORAGE
	version = Flash_Read(EEPROM_ADDR_VERSION);
	#else
	version = EEPROM_Read(EEPROM_ADDR_VERSION);
	#endif
  if (version == SETTINGS_VERSION) {
    // Read settings-record and check checksum
    if (!(read_from_storage((char*)&settings, EEPROM_ADDR_GLOBAL, sizeof(settings_t)))) {
      return(false);
    }
  } else {
    return(false); 
