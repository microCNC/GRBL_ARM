...
#endif
{
  ...
  inverse_millimeters = 1.0f/block->millimeters;  // Inverse millimeters to remove multiple float divides	
  junction_cos_theta = 0;
  for (idx=0; idx<N_AXIS; idx++) {
    if (unit_vec[idx] != 0) {  // Avoid divide by zero.
      unit_vec[idx] *= inverse_millimeters;  // Complete unit vector calculation
      inverse_unit_vec_value = fabs(1.0f/unit_vec[idx]); // Inverse to remove multiple float divides.
