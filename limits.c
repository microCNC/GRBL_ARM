...
void limits_init() 
{
  ...
  LIMIT_DDR &= ~(LIMIT_MASK); // Set as input pins
	LIMIT_ENABLE |= LIMIT_MASK; // Enable Limit Pins

  if (bit_istrue(settings.flags,BITFLAG_INVERT_LIMIT_PINS)) {
    LIMIT_PULLUP &= ~(LIMIT_MASK); // Normal low operation. Requires external pull-down.
  } else {
    LIMIT_PULLUP |= (LIMIT_MASK);  // Enable internal pull-up resistors. Normal high operation.
  }
  ...
  if (bit_istrue(settings.flags,BITFLAG_HARD_LIMIT_ENABLE)) {
		...
		NVIC_EnableIRQ(LIMIT_PORT_IRQN);						// enable IRQ
...
void limits_disable()
{
	LIMIT_PORT->IM  &= ~LIMIT_MASK;
	NVIC_DisableIRQ(LIMIT_PORT_IRQN);
}
...
void limits_go_home(uint8_t cycle_mask, bool approach, float homing_rate) 
{
  ...
  max_travel = settings.max_travel[X_AXIS];
